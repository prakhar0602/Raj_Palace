...
function linkFunction(item) {
    if(item=="Reservation" || (window.location.pathname==content["mb"][item].link) )
      return
    dispatch(setLoading(true))
    if (content["mb"][item].function) 
      setlist(content["mb"][item].function);
    else if (item == "Download MICE Brochure")
      window.open(content["mb"][item].link);
    else 
      navigate(content["mb"][item].link);
  }
